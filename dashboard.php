...
try {
    ...
    error_log('Dashboard stale hardware update: ' . $e->getMessage());
}
// System and hardware status query 
try {
...
// Latest greenhouse readings query 
$ghReadings = [];
...
// Recent greenhouse events and alerts query 
try {
...
// The dashboard mirrors the Reports live-event feed, but only for the
// researcher who owns the currently running experiment.
if (!$dashboardLockedByOtherExperiment) {
    $recentAlerts = array_slice(ecotwinBuildLiveIssueEvents($pdo), 0, 5);
} else {
    $recentAlerts = [];
    $criticalAlerts = [];
}
...
function severity_icon(string $sev): string
{
    return match ($sev) {
        ...
        'warning'  => '<span class="event-icon warning">&#9888;</span>',
        'success'  => '<span class="event-icon success">&#10003;</span>',
        default    => '<span class="event-icon info">i</span>',
    };
}
        ...
        <section class="summary-grid mb-3">
            ...
            <div class="summary-card">
                ...
                <?php elseif ($sensorsOffline > 0): ?>
                    <div class="summary-warning">&#9888; <?= $sensorsOffline ?> offline</div>
                <?php else: ?>
            ...
            <div class="summary-card">
                ...
                <?php elseif ($hwOffline > 0): ?>
                    <div class="summary-warning">&#9888; <?= $hwOffline ?> component(s) down</div>
                <?php endif; ?>
            </div>
            ...
            <section class="card analytics-overview-card mb-3">
                ...
                <div class="analytics-grid">
                    ...
                    <div class="analytics-stat">
                        ...
                        <div class="analytics-value"><?= isset($analyticsRow['avg_temperature']) && $analyticsRow['avg_temperature'] !== null ? e(number_format((float)$analyticsRow['avg_temperature'], 1)) . '&deg;C' : '-' ?></div>
                    </div>
                    <div class="analytics-stat">
                ...
                <div>
                    ...
                    <?php if (!empty($alert['gh_code'])): ?>
                        <a href="greenhouses.php" class="alert-link">
                            View Greenhouse <?= e($alert['gh_code']) ?> &rarr;
                        </a>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <?php if (!$dashboardLockedByOtherExperiment): ?>
            ...
            <section class="greenhouse-status-grid mb-3">
                <?php foreach ($greenhouses as $gh):
                    ...
                    // Flag critical if temp reading exceeds 30 or open critical alert exists
                    $tempCrit = $hasCrit;
                ?>
                    <div class="greenhouse-status-card <?= $cardCls ?>">
                            ...
                            <div class="param-item <?= $tempCrit ? 'critical' : '' ?>">
                                ...
                                <div>
                                    ...
                                    <div class="param-value">
                                        <?= $temp
                                            ? e(number_format((float)$temp['value'], 1)) . '&deg;C'
                        ...
                        <div class="gh-footer">
                            <a href="greenhouses.php" class="gh-link">View Details &rarr;</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </section>
