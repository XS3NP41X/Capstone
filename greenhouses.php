...
foreach ($greenhouses as $greenhouseRow) {
    ...
    $id = (int)$greenhouseRow['greenhouse_id'];
    $data[$code] = [
        'info'      => $gh_map[$code],
...
$current_user = [
    ...
    'initials' => strtoupper(implode('', array_map(fn($w) => $w[0], array_slice(explode(' ', trim($_SESSION['user_name'] ?? 'U')), 0, 2)))),
];
...
function renderGreenhousePanel(string $code, array $d, array $exp, bool $hasActiveExperiment): string {
    ...
    $role_label   = trim((string)($info['role'] ?? '')) !== ''
...
$greenhouseCodes = array_keys($data);
$defaultTab = $greenhouseCodes ? strtolower((string)$greenhouseCodes[0]) : '';
$active_tab = strtolower((string)($_GET['tab'] ?? $defaultTab));
if (!isset($data[strtoupper($active_tab)]) && $defaultTab !== '') {
...
$panels_html = '';
$rulesByCode = [];
foreach ($data as $code => $greenhouseData) {
    $lcCode = strtolower((string)$code);
    $isActive = $lcCode === $active_tab;
    $tabClass = $isActive ? 'tab-button active' : 'tab-button';
    $tabLabel = trim((string)($greenhouseData['info']['name'] ?? '')) !== ''
        ? (string)$greenhouseData['info']['name']
        : 'Greenhouse ' . $code;
    ...
    $panelHtml = preg_replace(
        ...
        '<div id="greenhouse-' . htmlspecialchars($lcCode) . '" class="' . $panelClass . '">',
        $panelHtml,
        1
    );
    $panels_html .= $panelHtml;
    $rulesByCode[$code] = array_values($greenhouseData['rules'] ?? []);
}

$rules_json = json_encode($rulesByCode, JSON_HEX_TAG);
